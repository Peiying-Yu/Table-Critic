...
def main(
    ...
    critic_results_dir: str = "results/critic/wikitq",
    base_url="",
    ...
    try:
        subprocess.run(["rm", "-r", os.path.join(critic_results_dir, "cache")], check=True)
        print(f"Delete {os.path.join(critic_results_dir, 'cache')}")
    except subprocess.CalledProcessError as e:
        print(f"Delete {os.path.join(critic_results_dir, 'cache')}: {e}")
